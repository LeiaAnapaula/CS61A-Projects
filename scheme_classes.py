...
class Frame:
    ...
    def define(self, symbol, value):
        ...
        # BEGIN PROBLEM 1
        self.bindings[symbol] = value # how do you attach in a dict?
        # put symbol inside the dict
        # END PROBLEM 1
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings:
            return self.bindings[symbol] # self and not self.uper() evaluates the first frame of the environment.
        
        elif symbol not in self.bindings and self.parent:
            return self.parent.lookup(symbol)
            
            # to call a parent, go from general to specific. Step by step :) literally
        # END PROBLEM 1
        raise SchemeError('unknown identifier: {0}'.format(symbol)) # here's the scheme error
