...
def roll_dice(num_rolls, dice=six_sided):
    ...
    #def dice(num_rolls): # what is the right function here?
    total = 0
    if_one = False
    k = 1
    while k <= num_rolls:
        num = dice() # need to attach the function to a number to extract the outcome
        if num != 1: # now, that single number
            total = total + num #call dice with ()
        else:
            if_one = not False # just = instead of ==
        k = k + 1
    if not if_one:
        return total
    else:
...
def tail_points(opponent_score):
    ...
    # opponent_score < GOAL and opponent_score >= 0:
    # get the tens of 123, 1345 get rid of the numbers to the side
    # tens number of second rightmost digit
    # ones always 
    tens = opponent_score // 10 # get us tens digit. 12, 134
    tens = tens % 10 # 2, 4 # remember % 10 also multiplies 0, so 6 % 10 = 6 cuz 10 x 0 is 0 and remainder is 6 

    ones = opponent_score % 10 # get us ones digit. This is good.
...
def perfect_square(score):
    d = 1
    while d <= score:
        if d * d == score: # need to store the digit
            return True
        d = d + 1
    return False
...
def next_perfect_square(score):
    ...
    while d <= score:
        ...
        # while the counter is less or equal than the number
        # if the multiplication of those first two numbers is the score we got
        # add one to that single number and square it
        # return the square
...
def play(strategy0, strategy1, update,
    ...
    # (1) Players take turns rolling dice until 
    # one of the players reaches the goal score
    # (2) returns the number of dice 
    # that the current player will roll in the turn
     
    
    #NOPE!
    

    while score0 < goal and score1 < goal: # yep, just less
        # take turns
        # increment the score accordingly and evaluat if it's the goal
        # if reaches the goal return You Win!
        num = strategy0(score0, score1) if who == 0 else strategy1(score1, score0) # we can put the if and else in one single line
        if who == 0:
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score < goal:
        opponent = 0
        while opponent < goal:
            currents = strategy(score, opponent)
            if currents != num:
                return False
            opponent += 1
        score += 1
...
def make_averaged(original_function, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def avg_dice(*args): # return value of make_average
        k = 0
        sum = 0.0
        while k < total_samples:
            right_now = original_function(*args)
            sum += right_now
            k += 1
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    # how to get the max average? isnt it just the average
    ...
    while k <= 10:
        num = make_averaged(roll_dice, total_samples)(k, dice) # takes on num_rolls = k, dice
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    # the zero that equals num_rolls' argument automatically mean calling on tail_points :)
    # so, write your known and unknowns when writing code always!
    ...
    # END PROBLEM 11
# take tail point and sum it to score
    # (maybe obvia esta parte) if it's a perfect square
        # apply square_update
    # Then, if (nested) square_update - score >= threshold
        # return0
    # else: (at the same indentation of the first if sttmnt)
        # return num_rolls
    # while square_update is True (not square) return (directly) num_rolls
    
    #while perfect_square(score) == True:
    #pig = tail_points(opponent_score) 
        #score = score + pig
